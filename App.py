"""FastAPI app for brain tumor image classification using Triton inference server."""
...
def resolve_host(hostname: str) -> str:
    """Resolve hostname to IP or return original hostname if resolution fails."""
    try:
...
def init_triton_client() -> bool:
    """Initialize Triton gRPC client with retries."""
    retries = 15
...
def init_s3_client() -> bool:
    """Initialize MinIO S3 client and verify bucket access."""
    try:
...
def convert_to_rgb(img: np.ndarray) -> np.ndarray:
    """Convert BGR or other image formats to RGB."""
    if len(img.shape) == 2:
...
def resize_and_normalize(img: np.ndarray) -> np.ndarray:
    """Resize image and normalize pixel values to [0,1], transpose to CHW."""
...
def preprocess_image(data: bytes) -> np.ndarray:
    """Preprocess image data: decode, convert, resize, normalize."""
